...
class CalendarSpreadStrategy(Strategy):
    ...
    def _load_snapshot(self, snapshot_id: int, db_conn: Any) -> dict:
        ...
        result = db_conn.execute(
            "SELECT * FROM snapshots_0dte WHERE id = %s", [snapshot_id]
        ).fetchone()
        if not result:
    ...
    def _evaluate_entry(self, snapshot: dict, db_conn: Any) -> dict:
        ...
        if not opportunity:
            return {"action": "NONE", "reason": "No opportunity signal computed yet"}
        ...
        if not violation or not violation["is_violation"]:
            return {"action": "NONE", "reason": "No active term structure violation"}
    ...
    def _evaluate_exit(self, position: dict, snapshot: dict, db_conn: Any) -> dict:
        ...
        )  # Debit spreads: profit = entry > current
        ...
        if pnl_pct >= self.PROFIT_TARGET_PCT:
            self._close_position(
                position["id"], current_price, snapshot, "profit_target", db_conn
            )
            return {
                "action": "CLOSE",
        ...
        if hold_days >= self.HOLD_TIME_DAYS:
            self._close_position(
                position["id"], current_price, snapshot, "theta_target", db_conn
            )
            return {
                "action": "CLOSE",
        ...
        if pnl_pct <= -self.STOP_LOSS_PCT:
            self._close_position(
                position["id"], current_price, snapshot, "stop_loss", db_conn
            )
            return {
                "action": "CLOSE",
        ...
        if violation and violation["iv_slope"] > 0:  # Inversion reversed
            self._close_position(
                position["id"], current_price, snapshot, "thesis_broken", db_conn
            )
            return {
                "action": "CLOSE",
        ...
        if hold_days >= self.MAX_HOLD_DAYS:
            self._close_position(
                position["id"], current_price, snapshot, "max_hold", db_conn
            )
            return {
                "action": "CLOSE",
    ...
    def _close_position(
        ...
        position = db_conn.execute(
            "SELECT * FROM calendar_spread_trades WHERE id = %s", [position_id]
        ).fetchone()

        final_pnl = position["entry_price"] - exit_price
        final_pnl_pct = (
            final_pnl / position["entry_price"] if position["entry_price"] > 0 else 0
        )
        hold_duration_days = (
            snapshot["captured_at"] - position["entry_timestamp"]
        ).days
    ...
    def _estimate_current_price(
        self, position: dict, snapshot: dict, db_conn: Any
    ) -> float:
        """
